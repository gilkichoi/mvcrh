...
const matchStr = "export const DEPARTMENTS: DetailedDepartment[] = [";
const startIndex = content.indexOf(matchStr);

// Let's find where 'opd' starts, we will append it there to keep OPD, radiology, laboratory from original 
const opdIndex = content.indexOf("  {\n    id: 'opd',", startIndex);

let updatedContent = content.substring(0, startIndex) + matchStr + '\n' + newDepartments + content.substring(opdIndex);

// Let's remove the old 'maternity', 'surgery', 'pharmacy' from the end of the original list or just completely rewrite the whole list.
